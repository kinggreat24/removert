...
// for target range of scan idx
{
    ...
    {
        ROS_INFO("scan path: %s",_scan_path.c_str());
...
void Removerter::map2RangeImg(const pcl::PointCloud<PointType>::Ptr &_scan,
...
{
    //Get current pose in Twc format
    Eigen::Matrix4d base_pose_inverse = scan_inverse_poses_.at(_base_scan_idx);
    // global to local (global2local)
    map_local_curr_->clear();
    ...
    int _base_scan_idx, pcl::PointCloud<PointType>::Ptr &_map_local)
{
    ...
    sprintf(file_name, "%s/local_map_camera_%d.pcd",map_static_save_dir_.c_str(),_base_scan_idx);
    pcl::io::savePCDFileASCII(file_name, *_map_local);
...
std::vector<int> Removerter::calcDescrepancyAndParseDynamicPointIdx(const cv::Mat &_scan_rimg, const cv::Mat &_diff_rimg, const cv::Mat &_map_rimg_ptidx)
{
...
std::vector<int> Removerter::calcDescrepancyAndParseDynamicPointIdxForEachScan(std::pair<int, int> _rimg_shape)
{
    ...
    for (std::size_t idx_scan = 0; idx_scan < scans_.size(); ++idx_scan)
    {
        ...
        char file_name[128]={0};
        sprintf(file_name, "%s/cur_%d.pcd",map_static_save_dir_.c_str(),idx_scan);
        pcl::io::savePCDFileASCII(file_name, *_scan);
        ...
        std::pair<cv::Mat, cv::Mat> map_rimg_pair;
        map2RangeImg(map_local_curr_, kFOV, _rimg_shape, map_rimg_pair); // the most time comsuming part 2 -> so openMP applied inside
        cv::Mat map_rimg = map_rimg_pair.first.clone();
        cv::Mat map_rimg_ptidx = map_rimg_pair.second.clone();
...
void Removerter::scansideRemovalForEachScan(void)
{
    ...
    for (std::size_t idx_scan = 0; idx_scan < scans_.size(); idx_scan++)
    {
        ...
        std::pair<pcl::PointCloud<PointType>::Ptr, pcl::PointCloud<PointType>::Ptr> this_scan_pair = removeDynamicPointsOfScanByKnn(idx_scan);
        pcl::PointCloud<PointType>::Ptr this_scan_static = this_scan_pair.first;
        pcl::PointCloud<PointType>::Ptr this_scan_dynamic = this_scan_pair.second;
