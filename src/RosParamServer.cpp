    ...
    : nh(nh_super), ROSimg_transporter_(nh)
{
    ...
    for(size_t i=0;i<kVecExtrinsicLiDARtoPoseBase.size();i++)
    {
        std::cout<<" "<<kVecExtrinsicLiDARtoPoseBase[i];
        if(i%4==0)
            std::cout<<std::endl;
    }    
    std::cout<<std::endl;
    ...
    DIR *pDir;
    struct dirent *ptr;
    if (!(pDir = opendir(sequence_scan_dir_.c_str())))
    {
    ...
    while ((ptr = readdir(pDir)) != 0)
    {
        ...
        {
            sequence_scan_names_.push_back(ptr->d_name);
            sequence_scan_paths_.push_back(sequence_scan_dir_ + "/" + ptr->d_name);
        }
    }
    closedir(pDir);
    std::sort(sequence_scan_names_.begin(), sequence_scan_names_.end());
    ...
    while (getline(pose_file_handle, strOneLine))
    {
        ...
        // save (move)
        // std::cout << "Pose of scan: " << sequence_scan_names_.at(num_poses) << std::endl;
        // std::cout << ith_pose << std::endl;
